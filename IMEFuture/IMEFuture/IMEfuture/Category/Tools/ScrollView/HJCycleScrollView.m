...
//        imageview.autoresizingMask =UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleWidth;
        ...
        if ([imageStr hasPrefix:@"http"]) {
            [imageview  sd_setImageWithURL:[NSURL URLWithString:imageStr] placeholderImage:[UIImage  imageNamed:@"ime_home_banner"]];
        }else{
...
- (void)refreshScrollView {
    ...
    if (_displayViews.count == 1) {
        ...
        [self.scrollView setContentOffset:CGPointMake(0, 0.0f)];
    } else if (_displayViews.count == 2) {
...
- (NSArray *)curDisplayViewsWithCurPage:(NSInteger)curPage {
    ...
    if (_displayViews.count == 1) {
        NSInteger backPage    = [self pageNumber:curPage];
        NSInteger forwardPage = [self pageNumber:curPage];
        ...
        [currentViews addObject:[self duplicate:[_displayViews objectAtIndex:backPage]]];
        [currentViews addObject:_displayViews[curPage]];
        [currentViews addObject: [self duplicate:[_displayViews objectAtIndex:forwardPage]]];
