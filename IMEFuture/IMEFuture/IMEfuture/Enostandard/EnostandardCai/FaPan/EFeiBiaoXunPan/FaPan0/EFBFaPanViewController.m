...
@interface EFBFaPanViewController () <UITableViewDelegate,UITableViewDataSource,UITextViewDelegate> {
    NSArray *_dataArray;
...
@implementation EFBFaPanViewController
...
- (void)viewDidLoad {
    ...
    self.inquiryOrder.allowTracking = [NSNumber numberWithInteger:0];//智造家人工推荐合适的供应商(免费)
    self.inquiryOrder.isPrivate = [NSNumber numberWithInteger:0];//匿名采购
...
//    _dataArray = @[@"询盘名称",@"目标价供应商可见",@"报价方式",@"交货日期",@"询盘天数",@"货运方式",@"询盘工艺"];
    _dataArray = @[@"目标价供应商可见",@"报价方式",@"交货日期",@"询盘天数",@"货运方式",@"询盘工艺",@"零件类型",@"加工类型",@"智造家人工推荐合适的供应商(免费)",@"匿名采购",@"期望供应商税率",@"是否加急"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {//目标价供应商可见
        ...
        return cell;
    } else if (indexPath.row < 8) {//交货日期～询盘工艺
        ...
        if (indexPath.row == 3) {//询盘天数
            cell.textField.text = _endTime;
            cell.textField.placeholder = @"请选择，必填项";
        }
        if (indexPath.row == 4) {//货运方式
            cell.textField.text = [NSString DeliveryMethod:self.inquiryOrder.deliveryMethod];
            cell.textField.placeholder = @"请选择，必填项";
        }
        if (indexPath.row == 5) {//询盘工艺
        ...
        cell.label0.text = _dataArray[indexPath.row];
        if (indexPath.row == 8) {//智造家人工推荐合适的供应商(免费)
            cell.switch0.on = [self.inquiryOrder.allowTracking integerValue] == 1?YES:NO;
        } else if (indexPath.row == 9) {//匿名采购
            cell.switch0.on = [self.inquiryOrder.isPrivate integerValue] == 1?YES:NO;
        }
        [cell.switch0 addTarget:self action:@selector(switchAction:) forControlEvents:UIControlEventValueChanged];
        cell.switch0.tag = indexPath.row;
        
        return cell;
    } else if (indexPath.row == 10) {//采购期望供应商税率
        EFBFaPanCell2 *cell = [tableView dequeueReusableCellWithIdentifier:@"eFBFaPanCell2" forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        cell.label0.text = _dataArray[indexPath.row];
        ...
        [cell.buttonTextField addTarget:self action:@selector(btnSupplierTaxRate:) forControlEvents:UIControlEventTouchUpInside];
        cell.buttonTextField.tag = indexPath.row;
...
- (void)btnSupplierTaxRate:(UIButton *)sender {
...
#pragma mark 目标价供应商可见、智造家人工推荐合适的供应商(免费)、匿名采购
...
#pragma mark 交货日期、询盘天数、货运方式、询盘工艺、零件类型、加工类型
- (void)buttonTextFieldClick:(UIButton *)sender {
    NSLog(@"----------%s",__FUNCTION__);
    if (sender.tag == 2) {//交货日期
...
//            NSDate *date = [NSDate date];
            ...
            self.inquiryOrder.inquiryDay = [NSNumber numberWithInteger:string1.integerValue];
            [self.tableView reloadRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:sender.tag inSection:0]] withRowAnimation:UITableViewRowAnimationNone];
        } buttonQuXiao:^{
            [sender resignFirstResponder];
        }];
    } else if (sender.tag == 4) {//货运方式
...
- (void)textViewDidChange:(UITextView *)textView {
    self.inquiryOrder.supplierRemark = textView.text;
    NSLog(@"优选供应商:%@",self.inquiryOrder.supplierRemark);
}
...
- (IBAction)buttonXiaYiBu:(UIButton *)sender {
    ...
    EFBFaPanViewController1 *eFBFaPanViewController1 = [[EFBFaPanViewController1 alloc] init];
    eFBFaPanViewController1.inquiryOrder = self.inquiryOrder;
    eFBFaPanViewController1.buttonBackBlock = ^(InquiryOrder *inquiryOrder) {
        self.inquiryOrder = inquiryOrder;
    };
    [self.navigationController pushViewController:eFBFaPanViewController1 animated:YES];
}
