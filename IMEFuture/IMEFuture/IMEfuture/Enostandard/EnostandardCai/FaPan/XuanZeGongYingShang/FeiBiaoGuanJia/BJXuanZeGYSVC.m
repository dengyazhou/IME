...
@interface BJXuanZeGYSVC () <UITableViewDelegate,UITableViewDataSource> {
    UITableView *_tableView0;
    
    NSMutableArray *_arrayEnterpriseRelation0;
    
    NSMutableArray *_arrayEnterpriseRelation012;//总数组
...
- (void)viewDidLoad {
    ...
    _arrayEnterpriseRelation012 = [NSMutableArray arrayWithCapacity:0];
    
    self.scrollView.contentSize = CGSizeMake(kMainW, 0);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (isNoSelect == NO) {
        if (self.arrayTempEnterpriseRelation.count >= 20) {
            [[MyAlertCenter defaultCenter] postAlertWithMessage:@"不能超过20家供应商"];
            return;
        }
        ...
        [HttpMamager postRequestWithURLString:DYZ_epRelation_trustRelationList parameters:dic success:^(id responseObjectModel) {
            ...
            if ([model.status isEqualToString:@"SUCCESS"]) {
                ...
                for (NSDictionary *dic in enterpriseRelations) {
                    ...
                    for (EnterpriseRelation *enterpriseRelation0123 in self->_arrayEnterpriseRelation012) {
                    ...
                    if (isNo == NO) {
                        [self->_arrayEnterpriseRelation012 addObject:enterpriseRelation];
                    }
                }
