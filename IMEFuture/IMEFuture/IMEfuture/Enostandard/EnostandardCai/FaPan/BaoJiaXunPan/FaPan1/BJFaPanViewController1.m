...
#import "Masonry.h"
#import "BJFaPanLingJianCell.h"

#import "BJFaPanViewController2.h"

#import "XinZengLingJianDuoCiVC.h"

@interface BJFaPanViewController1 () <UITableViewDelegate,UITableViewDataSource> {
...
@implementation BJFaPanViewController1
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"BJFaPanLingJianCell" bundle:nil] forCellReuseIdentifier:@"bJFaPanLingJianCell"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BJFaPanLingJianCell *cell = [tableView dequeueReusableCellWithIdentifier:@"bJFaPanLingJianCell" forIndexPath:indexPath];
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.viewYaoQiuDaoHuoRiQi.hidden = YES;
    
    if ([self.inquiryOrder.isPre integerValue] == 0) {
        cell.viewYaoQiuDaoHuoRiQi.hidden = YES;
    } else if ([self.inquiryOrder.isPre integerValue] == 1) {
        cell.viewYaoQiuDaoHuoRiQi.hidden = NO;
    }
...
- (IBAction)buttonXiaYiBu:(UIButton *)sender {
    ...
    if ([self.inquiryOrder.isPre integerValue] == 1) {//多次发货
        for (InquiryOrderItem *inquiryOrderItem in self.inquiryOrder.inquiryOrderItems) {
            if (inquiryOrderItem.sec_batchDeliverItem == nil || inquiryOrderItem.sec_batchDeliverItem.count == 0) {
                [[MyAlertCenter defaultCenter] postAlertWithMessage:@"请补全所有零件里的要求到货日期"];
                return;
            }
        }
    }
    
    BJFaPanViewController2 *bJFaPanViewController2 = [[BJFaPanViewController2 alloc] init];
    
    NSDictionary *dic = [self.inquiryOrder mj_keyValues];
    NSLog(@"%@",dic);
    
    bJFaPanViewController2.inquiryOrder = self.inquiryOrder;
    bJFaPanViewController2.buttonBackBlock = ^(InquiryOrder *inquiryOrder) {
        self.inquiryOrder = inquiryOrder;
    };
    [self.navigationController pushViewController:bJFaPanViewController2 animated:YES];
}
