...
@interface BuFaHuoVC () <UITableViewDelegate,UITableViewDataSource,UITextViewDelegate,UITextFieldDelegate> {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (section == 1) {
        ...
        faHuoHeaderView.label1.text = @"注：发货数量不得小于补发货数量";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        cell.label3.text = [NSString stringWithFormat:@"%ld",[orderOperateItem.reissueQuantity integerValue]];
        
        orderOperateItem.operateNum = orderOperateItem.reissueQuantity;
...
- (void)textFieldFaHuoShuLiangDidEnd:(UITextField *)sender {
    OrderOperateItem *orderOperateItem = _arrayOrderOperateItem[sender.tag];
    if ([orderOperateItem.reissueQuantity integerValue] > [orderOperateItem.operateNum integerValue]) {
        [[MyAlertCenter defaultCenter] postAlertWithMessage:@"发货数量不得小于补发货数量"];
        orderOperateItem.operateNum = orderOperateItem.reissueQuantity;
        [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:sender.tag+1] withRowAnimation:UITableViewRowAnimationNone];//刷新单独一个区
    }
}
...
- (void)textFieldOperateCode1DidEnd:(UITextField *)sender {
    ...
    [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:1+_arrayOrderOperateItem.count] withRowAnimation:UITableViewRowAnimationNone];
}
...
- (void)buttonClickXianXiaWuLiu:(UIButton *)sender {
    ...
    [viewXZWL initPickerViewButtonClick:^(NSString *string) {
        ...
        [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:1+_arrayOrderOperateItem.count+1] withRowAnimation:UITableViewRowAnimationNone];
    }];
}
...
- (IBAction)buttonClickQueRenFaHuo:(UIButton *)sender {
    ...
    [HttpMamager postRequestWithURLString:DYZ_i_orderOperate_reissueOrderOperate parameters:dic success:^(id responseObjectModel) {
...
//    BuFaHuoXiangQingVC *buFaHuoXiangQingVC = [[BuFaHuoXiangQingVC alloc] init];
//    [self.navigationController pushViewController:buFaHuoXiangQingVC animated:YES];
}
...
- (void)initRequest {
    ...
    [HttpMamager postRequestWithURLString:DYZ_i_orderOperate_reissueOperate parameters:dic success:^(id responseObjectModel) {
        ...
        if ([model.status isEqualToString:@"SUCCESS"]) {
            ...
            _orderOperate.logisticsCompanyKey = @"XX";
            _orderOperate.deliverNumber = nil;
            _orderOperate.logisticsNo = nil;
