...
@interface ChaKanShouPanHeJiaVC () <UITableViewDelegate,UITableViewDataSource,UIGestureRecognizerDelegate,UITextFieldDelegate,UITextViewDelegate>{
...
@implementation ChaKanShouPanHeJiaVC
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    if (tableView.tag == 88) {
        if (section == 0) {
            ...
            view.labelHeJia00.textColor = colorRGB(0, 168, 255);
            ...
            view.labelHeJia1.textColor = colorRGB(0, 168, 255);
            view.labelHeJia10.text = @"采购商比价";
            
            view.labelHeJia11.textColor = colorRGB(0, 168, 255);
            double totalPrice = 0;
...
//                cell.label0.text = [NSString stringWithFormat:@"总计(含税价%@%@)",stringTaxRate,@"%"];
            ...
            if (indexPath.row == 0) {
                ...
                cell.zongYiJialabel2.text = @"我的报价:";
                ...
                if (indexPath.row == 2+[_inquiryOrderHttp.tempPriceDetailCount integerValue]) {
                    ...
                    cell.textField1.textColor = colorRGB(0, 168, 255);
...
- (IBAction)buttonClickBuTongHuo:(UIButton *)sender {
    NSArray *nib = [[NSBundle mainBundle] loadNibNamed:@"UIViewGYSJiePanJuJueYuanYing" owner:self options:nil];
    UIViewGYSJiePanJuJueYuanYing *viewXZJPJJYY = [nib objectAtIndex:0];
    viewXZJPJJYY.frame = CGRectMake(0, 0, kMainW, kMainH);
    [self.view addSubview:viewXZJPJJYY];
    viewXZJPJJYY.backgroundColor = [[UIColor blackColor] colorWithAlphaComponent:0.3];
    [viewXZJPJJYY initPickerViewButtonClick:^(NSString *string) {
...
- (void)butongGuo:(NSString *)string {
    ...
    quotationOrder1.refuseId = loginModel.memberId;
    quotationOrder1.refuseName = loginModel.accountName;
    quotationOrder1.refuseMsg = string;
