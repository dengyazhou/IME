...
@interface YiCiFaHuoXiangQingVC () <UITableViewDelegate,UITableViewDataSource> {
...
@implementation YiCiFaHuoXiangQingVC
...
- (void)viewDidLoad {
    ...
    [self.tableView registerNib:[UINib nibWithNibName:@"YiCiFaHuoCell2" bundle:nil] forCellReuseIdentifier:@"yiCiFaHuoCell2"];
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    return 1+_orderOperate.orderOperateItems.count+2;
}

- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        return 83;
    } else if (indexPath.section < 1+_orderOperate.orderOperateItems.count) {
        return 120;
    } else if (indexPath.section == 1+_orderOperate.orderOperateItems.count) {
        return 257;
    } else if (indexPath.section == 1+_orderOperate.orderOperateItems.count+1) {
        return 89;
    }
    return 0;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section {
    if (section == 0) {
        ...
        return 31;
    } else if (section == 1+_orderOperate.orderOperateItems.count) {
        return 31;
    } else if (section == 1+_orderOperate.orderOperateItems.count+1) {
        return 31;
    }
    return 0;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ...
        YiCiFaHuoCell2 *cell = [tableView dequeueReusableCellWithIdentifier:@"yiCiFaHuoCell2" forIndexPath:indexPath];
        cell.selectionStyle = UITableViewCellSelectionStyleNone;
        OrderOperateItem *orderOperateItem = _orderOperate.orderOperateItems[indexPath.section-1];
        ...
        cell.label31.text = [NSString stringWithFormat:@"%ld",[orderOperateItem.tradeOrderItem.num integerValue]-[orderOperateItem.tradeOrderItem.deliverNums integerValue]>0?[orderOperateItem.tradeOrderItem.num integerValue]-[orderOperateItem.tradeOrderItem.deliverNums integerValue]:0];
        cell.label32.text = [NSString stringWithFormat:@"%ld",[orderOperateItem.operateNum integerValue]];
