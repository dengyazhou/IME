...
@interface LingJianXiangQingDingDanViewController2 () <WKNavigationDelegate>{
...
@implementation LingJianXiangQingDingDanViewController2
...
- (void)viewDidLoad {
    ...
    if ([self.isMatchDrawingCloud integerValue] == 0) {
        ...
        if ([self.tradeOrderItemFile.thumbnailUrl containsString:@".dwg"] ) {
            self.imageView.image = [UIImage imageNamed:@"picture_dwg"];
        } else if ([self.tradeOrderItemFile.thumbnailUrl containsString:@".png"] || [self.tradeOrderItemFile.thumbnailUrl containsString:@".gif"] || [self.tradeOrderItemFile.thumbnailUrl containsString:@".jpeg"]) {
            
            _activityIndicatorView.hidden = NO;
            [_activityIndicatorView startAnimating];
            [self.imageView sd_setImageWithURL:[NSURL URLWithString:self.tradeOrderItemFile.sec_realUrl] placeholderImage:[UIImage imageNamed:@"img_picture_conversion"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                [_activityIndicatorView stopAnimating];
            }];
        } else if ([self.tradeOrderItemFile.thumbnailUrl containsString:@".jpg"]) {
            if ([self.tradeOrderItemFile.thumbnailUrl containsString:@"pdf.jpg"]) {
                self.wkWebView.hidden = NO;
                _activityIndicatorView.hidden = NO;
                [_activityIndicatorView startAnimating];
                [self.wkWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.tradeOrderItemFile.sec_realUrl]]];
            } else {
