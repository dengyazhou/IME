//
//  PadMultiUserWorkCompleteVC.swift
//  IMEFuture
//
...
class PadMultiUserWorkCompleteVC: UIViewController,UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.tableView.register(UINib.init(nibName: "PadSingleUserWorkCompleteCell1", bundle: nil), forCellReuseIdentifier: "padSingleUserWorkCompleteCell1")
        ...
        self.tableView.register(UINib.init(nibName: "PadSingleUserWorkCompleHeader", bundle: nil), forHeaderFooterViewReuseIdentifier: "padSingleUserWorkCompleHeader")
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        if section == 0 {
            let view = tableView.dequeueReusableHeaderFooterView(withIdentifier: "padSingleUserWorkCompleHeader") as! PadSingleUserWorkCompleHeader
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 0 {
            let cell = tableView.dequeueReusableCell(withIdentifier: "padSingleUserWorkCompleteCell1", for: indexPath) as! PadSingleUserWorkCompleteCell1
            cell.selectionStyle = .none
            cell.label0?.text = self.multiUserWorkVo.materialText + "-" + self.multiUserWorkVo.materialCode
            cell.label1?.text = self.multiUserWorkVo.operationText
            cell.label2?.text = self.multiUserWorkVo.workUnitText
            cell.label3?.text = self.multiUserWorkVo.planQuantity.stringValue
            cell.label4?.text = FunctionDYZ.init().strDateFormat(self.multiUserWorkVo.requirementDate, withEnterDateFormat: "yyyy-MM-dd HH:mm:ss", withGoDateFormat: "yyyy-MM-dd")
...
// MARK: - 保留一位小数
    ...
    @objc func continueCheck() {
        ...
        if personnerlCode != "(null)" {//绑定了人员
            ...
            let vc = PadSelectPersonnelCheckListViewController()
        ...
        HttpMamager.postRequestImage(withURLString: DYZ_multiUserWork_doMultiUserConfirmProduction, parameters: dic2, uploadImageBean: imageArray, success: { (responseObjectModel) in
            ...
            if returnMsgBean.status == "SUCCESS" {
                ...
                for vc in (self.navigationController?.viewControllers)! {
                    if vc is PadProductionWorkViewController {
...
//                }
    ...
    @IBAction func back(_ sender: Any) {
        for temp in (self.navigationController?.viewControllers)! {
            if temp is PadProductionWorkViewController {
