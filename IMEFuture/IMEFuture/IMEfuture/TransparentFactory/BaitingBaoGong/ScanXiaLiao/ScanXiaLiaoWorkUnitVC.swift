...
class ScanXiaLiaoWorkUnitVC: UIViewController,UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        if textField.text?.count == 0 {
            MyAlertCenter.default().postAlert(withMessage: "请输入作业单元")
            return true
        }
        ...
        saoYiSao.resultBlock = {(result: String?) -> Void in
            ...
            if let a = dic {
                if let workUnitCode  = a["workUnitCode"] as? String {
                    self.goNextStep(result: workUnitCode)
                } else {
    ...
    func goNextStep(result: String) {
        self.blankingWorkTimeLogVo.workUnitCode = result
        if self.blankingWorkTimeLogVo.confirmUser.count == 0 {
