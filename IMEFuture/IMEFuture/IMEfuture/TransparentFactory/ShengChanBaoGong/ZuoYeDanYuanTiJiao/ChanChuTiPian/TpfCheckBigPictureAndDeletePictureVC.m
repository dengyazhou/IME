...
@interface TpfCheckBigPictureAndDeletePictureVC () <UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout> {
...
@implementation TpfCheckBigPictureAndDeletePictureVC
...
- (void)viewDidLoad {
    ...
    self.labelTitle.text = [NSString stringWithFormat:@"%ld/%ld",self.index+1,self.arrayUploadImageBean.count];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return self.arrayUploadImageBean.count;
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.imageView1.image = [UIImage imageWithData:self.arrayUploadImageBean[indexPath.row].data];
...
- (void)buttonCancelClick:(UIButton *)sender {
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        [self.arrayUploadImageBean removeObjectAtIndex:sender.tag];
        [_collectionView reloadData];

        self.labelTitle.text = [NSString stringWithFormat:@"%ld/%ld",self.index+1,self.arrayUploadImageBean.count];
        if (self.arrayUploadImageBean.count == 0) {
