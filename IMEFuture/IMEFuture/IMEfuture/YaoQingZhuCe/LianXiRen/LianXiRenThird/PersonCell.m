...
//        self.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageNamed:@"cellviewbackground"]];
        ...
        _txtName.textColor = colorRGB(51, 51, 51);
        ...
        _phoneNum.textColor = colorRGB(153, 153, 153);
        [self.contentView addSubview:_phoneNum];
        
        UILabel *buttonLabel = [[UILabel alloc] init];
        buttonLabel.frame = CGRectMake(kMainW-75, 17.5, 47, 25);
        buttonLabel.layer.borderWidth = 0.5;
        buttonLabel.layer.cornerRadius = 3;
        buttonLabel.layer.borderColor = colorRGB(89, 178, 50).CGColor;
        buttonLabel.text = @"添加";
        buttonLabel.font = [UIFont systemFontOfSize:12];
        buttonLabel.textColor = colorRGB(89, 178, 50);
...
-(void)setTxcolorAndTitle:(NSString*)title :(NSString*)fid
{
    ...
    if(title.length!=0)
    {
//       title= title.length<2? [title substringToIndex:title.length]:[title substringToIndex:2];//前两位
        title= title.length<2? [title substringToIndex:title.length]:[title substringFromIndex:title.length-2];//最后两位
    }else
    {
//       title=@"测试";
        title=@"";
