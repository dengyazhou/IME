...
@interface ShanChuTuPianVC () <UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout> {
...
@implementation ShanChuTuPianVC
...
- (void)viewDidLoad {
    ...
    self.labelTitle.text = [NSString stringWithFormat:@"%ld/%ld",self.index+1,self.inquiryOrderItem.inquiryOrderItemFiles.count];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    return self.inquiryOrderItem.inquiryOrderItemFiles.count;
}
...
- (void)buttonCancelClick:(UIButton *)sender {
    ...
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        InquiryOrderItemFile *inquiryOrderItemFile = self.inquiryOrderItem.inquiryOrderItemFiles[sender.tag];
        [self.inquiryOrderItem.inquiryOrderItemFiles removeObjectAtIndex:sender.tag];
        [_collectionView reloadData];
        
        self.labelTitle.text = [NSString stringWithFormat:@"%ld/%ld",self.index+1,self.inquiryOrderItem.inquiryOrderItemFiles.count];
        if (self.inquiryOrderItem.inquiryOrderItemFiles.count == 0) {
